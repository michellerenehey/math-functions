...
export function sumArrayWithThreeNumbers(sumArr) {
    const sumArrSum = sum(sum(sumArr[0], sumArr[1])[0], sumArr[2])[0];
    const sumArrStr = `${sumArr[0]},${sumArr[1]},${sumArr[2]} was passed in as an array of numbers, and ${sumArrSum} is their sum.`;
    return [sumArrSum, sumArrStr];
}
...
export function multiplyArrayWithThreeNumbers(multArr) { //eslint-disable-line
    const multArrMult = multiply(multiply(multArr[0], multArr[1])[0], multArr[2])[0];
    const multArrStr = `The numbers ${multArr[0]},${multArr[1]},${multArr[2]} have a product of 24.`;
    return [multArrMult, multArrStr];
}
...
export function multiplyAnyArray(dynamicArray) { //eslint-disable-line
    ...
    for(let num of dynamicArray) {
        result = multiply(result, num)[0];
    } 
    const dynamicArrayStr = `The numbers ${dynamicArray} have a product of ${result}.`;
    return [result, dynamicArrayStr];
}


//     const dynamicArrayStr = `The numbers 1,2,3,4,5 have a product of 120.`;
//     return [dynamicArrayMult, dynamicArrayStr];
// }
